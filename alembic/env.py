...
Pulls the database URL from `app.config.settings` (your .env file) and
the table metadata from `app.database.Base` + `app.models`, so the app's
models are always the single source of truth for `alembic revision
--autogenerate`. You never hand-write the DB URL a second time here.
"""
import os
import sys
